...
This program achieves both those issues. It runs as a Windows task bar item and links itself with the shortcut keys Win + N. This can be triggered from any program on the computer. Doing this will start a timer. When doing Win + N again you'll get a small prompt to choose the activity you just timed. These are set up ahead of time from the Activities menu. This is then logged to the database. 
